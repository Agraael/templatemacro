...
export function cellOccludedByTerrain(worldX, worldY, baseElev, range) {
  ...
  for (const s of shapes) {
    ...
    let type = null;
    if (typeId) {
      try { type = tht.getTerrainType?.({ id: typeId }); } catch { type = null; }
    }
    if (!type && s?.terrainType?.isSolid !== undefined) type = s.terrainType;
    ...
    if (sTop > ceiling) return true;
  }
  return false;
}

export function isInsideInnerCircle(templateDoc, worldX, worldY) {
  if (templateDoc?.t !== "circle") return false;
  ...
  for (let dx = -0.5; dx <= 0.5; dx += 0.5) {
    for (let dy = -0.5; dy <= 0.5; dy += 0.5) {
      if (shape.contains(lx + dx, ly + dy)) return true;
    }
  }
  return false;
}

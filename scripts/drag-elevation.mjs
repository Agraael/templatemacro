// Keybindings to adjust the elevation of a template preview/clone during drag.
// Works for both new placements (drag from measure tool) and moves of existing templates.
...
function _onPreCreateTemplate(doc) {
  if (_pendingElevation === null) return;
...
function _onPreUpdateTemplate(doc, changes) {
  if (_pendingElevation === null || _pendingOriginalId === null) return;
  if (doc.id !== _pendingOriginalId) return;
